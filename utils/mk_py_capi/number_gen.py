...
class NumberGen(Number):
    ...
    def __new__(cls, *,
        ...
        if nb_add is not None:
            nb_add = BinaryFuncGen('nb_add', nb_add)
        if nb_subtract is not None:
            nb_subtract = BinaryFuncGen('nb_subtract', nb_sbtract)
        if nb_multiply is not None:
            nb_multiply = BinaryFuncGen('nb_multiply', nb_multiply)
        if nb_remainder is not None:
            nb_remainder = BinaryFuncGen('nb_remainder', nb_remainder)
        if nb_divmod is not None:
            nb_divmod = BinaryFuncGen('nb_divmod', nb_divmod)
        if nb_power is not None:
            nb_power = TernaryFuncGen('nb_power', nb_power)
        if nb_negative is not None:
            nb_negative = UnaryFuncGen('nb_negative', nb_negative)
        if nb_positive is not None:
            nb_positive = UnaryFuncGen('nb_positive', nb_positive)
        if nb_absolute is not None:
            nb_absolute = UnaryFuncGen('nb_absolute', nb_absolute)
        if nb_bool is not None:
            nb_bool = InquiryGen('nb_bool', nb_bool)
        if nb_invert is not None:
            nb_invert = UnaryFuncGen('nb_invert', nb_invert)
        if nb_lshift is not None:
            nb_lshift = BinaryFuncGen('nb_lshift', nb_lshft)
        if nb_rshift is not None:
            nb_rshift = BinaryFuncGen('nb_rshift', nb_rshift)
        if nb_and is not None:
            nb_and = BinaryFuncGen('nb_and', nb_and)
        if nb_xor is not None:
            nb_xor = BinaryFuncGen('nb_xor', nb_xor)
        if nb_or is not None:
            nb_or = BinaryFuncGen('nb_or', nb_or)
        if nb_int is not None:
            nb_int = UnaryFuncGen('nb_int', nb_int)
        if nb_float is not None:
            nb_float = UnaryFuncGen('nb_float', nb_float)
        if nb_inplace_add is not None:
            nb_inplace_add = BinaryFuncGen('nb_inplace_add',
                                           nb_inplace_add)
        if nb_inplace_subtract is not None:
            nb_inplace_subtract = BinaryFuncGen('nb_inplace_subtract',
                                                nb_inplace_sbtract)
        if nb_inplace_multiply is not None:
            nb_inplace_multiply = BinaryFuncGen('nb_inplace_multiply',
                                                nb_inplace_multiply)
        if nb_inplace_remainder is not None:
            nb_inplace_remainder = BinaryFuncGen('nb_inplace_remainder',
                                                 nb_inplace_remainder)
        if nb_inplace_power is not None:
            nb_inplace_power = TernaryFuncGen('nb_inplace_power',
                                              nb_inplace_power)
        if nb_inplace_lshift is not None:
            nb_inplace_lshift = BinaryFuncGen('nb_inplace_lshift',
                                              nb_inplace_lshft)
        if nb_inplace_rshift is not None:
            nb_inplace_rshift = BinaryFuncGen('nb_inplace_rshift',
                                              nb_inplace_rshift)
        if nb_inplace_and is not None:
            nb_inplace_and = BinaryFuncGen('nb_inplace_and',
                                           nb_inplace_and)
        if nb_inplace_xor is not None:
            nb_inplace_xor = BinaryFuncGen('nb_inplace_xor',
                                           nb_inplace_xor)
        if nb_inplace_or is not None:
            nb_inplace_or = BinaryFuncGen('nb_inplace_or',
                                          nb_inplace_or)
        if nb_floor_divide is not None:
            nb_floor_divide = BinaryFuncGen('nb_floor_divide',
                                            nb_floor_divide)
        if nb_true_divide is not None:
            nb_true_divide = BinaryFuncGen('nb_true_divide',
                                           nb_true_divide)
        if nb_inplace_floor_divide is not None:
            nb_inplace_floor_divide = BinaryFuncGen('nb_inplace_floor_divide',
                                                    nb_inplace_floor_divide)
        if nb_inplace_true_divide is not None:
            nb_inplace_true_divide = BinaryFuncGen('nb_inplace_true_divide',
                                                   nb_inplace_true_divide)
        if nb_index is not None:
            nb_index = UnaryFuncGen('nb_index', nb_index)
        if nb_matrix_multiply is not None:
            nb_matrix_multiply = BinaryFuncGen('nb_matrix_multiply',
                                               nb_matrix_multiply)
        if nb_inplace_matrix_multiply is not None:
            nb_inplace_matrix_multiply = BinaryFuncGen('nb_inplace_matrix_multiply',
                                                       nb_inplace_matrix_multiply)
